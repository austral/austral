...
let check_slots_are_free (slots: typed_slot list): unit =
  let check_slot_is_free (slot: typed_slot): unit =
    ...
    | RegionUniverse ->
       ()
    | LinearUniverse ->
       err "Record was declared to belong to the Free universe, but it contains a type that beloings to the Linear universe."
    | TypeUniverse ->
       err "Record was declared to belong to the Free universe, but it contains a type that beloings to the Type universe."
  in
