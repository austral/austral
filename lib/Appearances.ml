...
and appear_row = Row of identifier * loop_context * appear list
...
let get_var (tbl: appear_tbl) (name: identifier): appear_row option =
  List.find_opt (fun (Row (name', _, _)) -> equal_identifier name name') tbl
...
let pop_var (tbl: appear_tbl) (name: identifier): (appear_row * appear_tbl) option =
  ...
  | Some row ->
     Some (row, List.filter (fun (Row (name', _, _)) -> not (equal_identifier name name')) tbl)
  | None ->
     None

let register_var (tbl: appear_tbl) (name: identifier) (ctx: loop_context): appear_tbl =
  match get_var tbl name with
  | Some _ ->
...
let register_appear (tbl: appear_tbl) (name: identifier) (pos: pos) (kind: appear_kind) (ctx: loop_context): appear_tbl =
  ...
  | Some (Row (_, init_ctx, appearances), tbl) ->
     (Row (name, init_ctx, Appear (pos, kind, ctx) :: appearances)) :: tbl
  | None ->
