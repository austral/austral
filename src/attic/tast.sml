...
structure TAST :> TAST = struct
    ...
    datatype top_ast = Defun of name * param list * ty * docstring * ast
                     ...
                     | Defdatatype of name * Type.kind * Type.typarams * docstring * Type.variant list
                     | Defrecord of name * Type.kind * Type.typarams * docstring * (name * ty) list
    ...
    in
        ...
        fun augment AST.UnitConstant _ = UnitConstant
          ...
          | augment (AST.ReadSlot (r, name)) c =
            ...
            in
                case typeOf r' of
                    (Type.Record (tyname, kind, _)) => let val slots = Type.getRecordSlots (ctxTenv c)
                                                                                           tyname
                                                       in
          ...
          | augment (AST.AddressOf name) c =
            (case (Map.get (ctxBindings c) name) of
                 (SOME (Binding (ty, Mutable))) => AddressOf (name, PositiveAddress ty)
